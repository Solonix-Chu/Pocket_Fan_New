...
void DetailView::setItems(const std::vector<std::string>& items)
{
    _items = items;
}

void DetailView::updateItem(int index, const std::string& text)
{
...
void DetailView::playEntryAnimation()
{
    for (size_t i = 0; i < _item_transitions.size(); i++) {
        auto& t = _item_transitions[i];
        const int y = 2 + static_cast<int>(i) * (_item_h + _item_gap);
        t.setDurationMs(360);
        t.setDelayMs(50 * i);
...
void DetailView::update()
{
    ...
    for (size_t i = 0; i < _item_labels.size(); i++) {
        ...
        lv_obj_set_pos(_item_labels[i], static_cast<int32_t>(pos.x) + 4, static_cast<int32_t>(pos.y));
    }

    lv_obj_set_pos(_list_cont, 0, -static_cast<int32_t>(_scroll_offset));
}

void DetailView::_create_lvgl_objects()
{
    if (_screen) {
        return;
    }
    ...
    if (!_items.empty()) {
        _content_height = 2 + static_cast<float>(_items.size() - 1) * (_item_h + _item_gap) + _item_h;
    } else {
        _content_height = 0.0f;
    }
}

void DetailView::setScrollOffset(float offset)
{
...
float DetailView::getContentHeight() const
{
